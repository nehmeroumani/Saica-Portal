...
class AnnotationForm extends Component {
  ...
  async fetchData(id) {
    ...
    let selectedWords = [];
    // Load annotationTaskUserTweet to set the dimensitons and reasons that are pre-selected - choosing words and answers
    if (annotationTaskUserTweet.annotations) {
      // check from annotations 
      annotationTaskUserTweet.annotations.forEach(annotation => {

        for (var c in validCategories) {
          var category = validCategories[c];

          if (category.id === annotation.categoryId) {
            // setting a selected flag in the categories array
            category.annotationId = annotation.id;
            // pre-select the dimension annotation
            //category.selectedDimensionId = annotation.dimensionId;
            category.selectedDimensionIds.push(annotation.dimensionId);
  ...
  detectKey(event) {
    ...
    if (event.keyCode === 27) {
      ...
      if (!selectedCategory.selectedDimensionId) {
        ...
        if (this.state.showDimensionKeyboard) {
          if (!isNaN(this.dimensionKeyboardInput.value)) {
            ...
            if (input <= selectedCategory.dimensions.length) {
              if (input > 0) {
                selectedCategory.selectedDimensionId = selectedCategory.dimensions[input - 1].id;
              }
              this.saveAnnotation(selectedCategory);
            }
          }
        }
  ...
  selectDimension(e, dimension, index) {
    e.preventDefault();
    this.setState({ selectedDimensionIndex: index, selectedDimension: dimension })
  }
  ...
  saveReason(selectedCategory, currentSelectedWords) {
    ...
    if (selectedCategory.selectedDimensionId) {
      ...
      const annotationreason = {
        annotationId: parseInt(selectedCategory.annotationId),
        categoryId: parseInt(selectedCategory.id),
        dimensionId: parseInt(selectedCategory.selectedDimensionId),
  ...
  saveAnnotation(selectedCategory) {
    ...
    if (selectedCategory.selectedDimensionId) {
      ...
      // get the data 
      const annotation = {
        id: selectedCategory.annotationId ? parseInt(selectedCategory.annotationId) : 0,
      ...
      dataProvider(CREATE, 'annotation', { id: annotation.id, data: annotation })
        .then(({ data }) => {
          selectedCategory.annotationId = data.id;
    ...
    return <li completed={completed} key={'category' + category.id} selectedItem={selected}>
      <a href="#" onClick={(e) => this.selectCategory(e, category, index)} style={{ position: "relative" }}>
        <span className="step justify-content-between full-row">
          ...
          {category.selectedDimensionId > 0 &&
  ...
  selectWord(word) {
    let selectedCategory = this.state.selectedCategory;
    console.log(selectedCategory.selectedDimensionId);
    if (selectedCategory.selectedDimensionId > 0) {
  ...
  deSelectWord(e, word, index) {
    ...
    // let selectedWords = this.state.selectedWords;
    // selectedWords = selectedWords.filter(x => x.id != word.id);
  ...
  deleteAnnotation(currentCategory) {
    analytics.logEvent('Annotation', 'Delete');
    const { dataProvider, reset } = this.props;

    fetchStart();
    dataProvider(DELETE, 'annotation', { id: currentCategory.annotationId })
      .then(({ data }) => {
        let selectedWords = this.state.selectedWords.filter(x => x.categoryId != currentCategory.id);
        currentCategory.annotationId = null;
        currentCategory.selectedWords = [];

        currentCategory.selectedDimensionId = 0;
  ...
  handleDimensionChange = event => {
    let selectedCategory = this.state.selectedCategory;
    selectedCategory.selectedDimensionId = event.target.value;
    this.saveAnnotation(selectedCategory);
    //this.setState({ selectedCategory: selectedCategory });
  };
    ...
    return (
      <div className={'sp-dimension-container'} style={{ marginTop: '22px' }}>
        {selectedCategory.id > 0 &&
          <Card  >
            <CardHeader
              // title={this.props.translate('resources.dimension.choose')}
              title={selectedCategory.name}
              //subheader="(إضغط d + رقم الجواب)"
            />
            <CardContent style={{ paddingTop: 0, paddingBottom: 0,paddingRight: 0, paddingLeft: 0 }}>
              <div>
                {/* <FormControl component="fieldset" className={classes.formControl}>
                  <RadioGroup
                    ...
                    name="gender1"
                    className={classes.group}
                    value={selectedCategory.selectedDimensionId + ''}
                    onChange={this.handleDimensionChange}
                  >
                    {this.state.showDimensionKeyboard && <input ref={(input) => { this.dimensionKeyboardInput = input; }} type="number" className="keyboard-words" />}
                    {selectedCategory.dimensions.map((c, i) => (
                      <FormControlLabel value={c.id + ''} control={<Radio />} label={(i + 1) + '. ' + c.name} style={{ marginLeft: '0px' }} />
                    ))}
                  </RadioGroup>
                </FormControl> */}
...
{selectedCategory.dimensions.map((d, i) =>   {
                  ...
                  var selected = this.state.selectedDimensions && this.state.selectedDimensions.includes(d.id) ? "selected" : null;

                  return(<div className="dimensionitem" 
                    selectedItem={selected}                    
                    onClick={(e) => this.selectDimension(e, d, index)} style={{ position: "relative" }}>
                      {(i + 1) + '. ' + d.name}</div>
                    )})}
                    
              </div>
               {/* <div>
                {this.state.annotationTaskUserTweet.status != 30 && this.state.selectedCategory && this.state.selectedCategory.selectedDimensionId > 0 &&
                  <Button size="small" onClick={this.handleDeleteAnnotation} color="primary"  >
                    {this.props.translate('resources.annotations.removeDimension')}
                  </Button>}
              </div> */}
            </CardContent>
          </Card>
        ...
        {selectedCategory.id == -1 &&
          <Card  >
            <CardHeader
              title={this.props.translate('resources.dimension.chooselevel')}
              subheader="*"
            />
            <CardContent style={{ paddingTop: 0, paddingBottom: 0 }}>
              <FormControl component="fieldset" className={classes.formControl}>
                ...
                >
                  {selectedCategory.dimensions.map((c, i) => (
                    <FormControlLabel value={c.id + ''} control={<Radio />} label={(i + 1) + ' ' + c.name} />
                  ))}
                </RadioGroup>
              </FormControl>
            </CardContent>

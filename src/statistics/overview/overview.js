...
class Overview extends Component {
    ...
    async fetchPieChart(filter) {
        try {
            const { json: data } = await httpClient(`${Config.apiUrl}/api/annotation/statistics`, {
                method: 'GET',
            });
    ...
    preparePieChartData(data) {
        var seriesData = [], drilldownSeriesData = [];
        if (data.annotationStatistics && data.annotationStatistics.length > 0) {
            if (data.categories) {
                const all = data.annotationStatistics.reduce((total, a) => {
                    total += a.total;
                    return total;
                }, 0);
                Object.getOwnPropertyNames(data.categories).forEach((categoryId) => {
                    seriesData.push({
                        name: data.categories[categoryId].name,
                        color: data.categories[categoryId].color,
                        drilldown: 'category_' + categoryId,
                        y: 100 * data.annotationStatistics.reduce((total, a) => {
                    ...
                    var categoryStats = data.annotationStatistics.filter((a) => {
                        return a.categoryId == categoryId;
                    });
                    const categoryTotal = categoryStats.reduce((total, a) => {
                        total += a.total;
                        return total;
                    }, 0);
                    ...
                    Object.getOwnPropertyNames(data.subCategories).forEach((subCategoryId) => {
                        const subCategoryStats = {
                            name: data.subCategories[subCategoryId].name,
                            y: 100 * categoryStats.reduce((total, a) => {
                                if (a.dimensionId == subCategoryId) {
                                    total += a.total;
                                }
                                return total;
                            }, 0) / categoryTotal
                        }
                        if (subCategoryStats.y > 0) {
                            categoryDrilldown.data.push(subCategoryStats);
                        }
                    });
                    drilldownSeriesData.push(categoryDrilldown);
                });
            }
        }
        this.setState({ pieChartData: { seriesData, drilldownSeriesData } });
    }

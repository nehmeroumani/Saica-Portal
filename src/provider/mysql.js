...
export default (apiUrl, httpClient = fetchUtils.fetchJson) => {
    ...
    const convertFilters = (filters) => {
        ...
        Object.keys(filters).map(function (key) {
            switch (typeof filters[key]) {
                ...
                case 'undefined':
                    rest['filter'] = key + ',eq,null';
                    break;

                case 'number':
                    rest['filter'] = key + ',eq,' + filters[key];
                    break;
    ...
    const convertDataRequestToHTTP = (type, resource, params) => {
        ...
        switch (type) {
            case GET_LIST: {
                ...
                let query = 'page=' + page + ',' + perPage + '&order=' + field + ',' + order.toLowerCase();
                if (f)
                    query += '&' + f;

                url = `${apiUrl}/records/${resource}?${query}`;
                break;
            }
            case GET_ONE:
                url = `${apiUrl}/records/${resource}/${params.id}`;
                break;
            // case GET_MANY: {
            //     const query = {
    ...
    return (type, resource, params) => {
        ...
        if (type === UPDATE_MANY) {
            return Promise.all(
                params.ids.map(id =>
                    httpClient(`${apiUrl}/records/${resource}/${id}`, {
                        method: 'PUT',
